/*
 * Algod REST API.
 *
 * API endpoint for algod operations.
 *
 * The version of the OpenAPI document: 0.0.1
 * Contact: contact@algorand.com
 * Generated by: Rust OpenAPI Generator
 */

use crate::models;
use algod_client::models::SimulationOpcodeTraceUnit as RustSimulationOpcodeTraceUnit;
use algokit_transact_ffi::SignedTransaction as AlgokitSignedTransaction;

use crate::models::ApplicationStateOperation;
use crate::models::AvmValue;
use crate::models::ScratchChange;

/// The set of trace information and effect from evaluating a single opcode.
#[derive(Clone, Default, Debug, PartialEq, uniffi::Record)]
pub struct SimulationOpcodeTraceUnit {
    /// The program counter of the current opcode being evaluated.
    pub pc: u64,
    /// The writes into scratch slots.
    pub scratch_changes: Option<Vec<ScratchChange>>,
    /// The operations against the current application's states.
    pub state_changes: Option<Vec<ApplicationStateOperation>>,
    /// The indexes of the traces for inner transactions spawned by this opcode, if any.
    pub spawned_inners: Option<Vec<u64>>,
    /// The number of deleted stack values by this opcode.
    pub stack_pop_count: Option<u64>,
    /// The values added by this opcode to the stack.
    pub stack_additions: Option<Vec<AvmValue>>,
}

impl From<RustSimulationOpcodeTraceUnit> for SimulationOpcodeTraceUnit {
    fn from(rust_struct: RustSimulationOpcodeTraceUnit) -> Self {
        Self {
            pc: rust_struct.pc.into(),
            scratch_changes: rust_struct.scratch_changes.into(),
            state_changes: rust_struct.state_changes.into(),
            spawned_inners: rust_struct.spawned_inners.into(),
            stack_pop_count: rust_struct.stack_pop_count.into(),
            stack_additions: rust_struct.stack_additions.into(),
        }
    }
}

impl From<SimulationOpcodeTraceUnit> for RustSimulationOpcodeTraceUnit {
    fn from(ffi_struct: SimulationOpcodeTraceUnit) -> Self {
        Self {
            pc: ffi_struct.pc.into(),
            scratch_changes: ffi_struct.scratch_changes.into(),
            state_changes: ffi_struct.state_changes.into(),
            spawned_inners: ffi_struct.spawned_inners.into(),
            stack_pop_count: ffi_struct.stack_pop_count.into(),
            stack_additions: ffi_struct.stack_additions.into(),
        }
    }
}
