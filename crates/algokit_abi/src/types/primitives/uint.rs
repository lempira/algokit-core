...
impl ABIType {
    pub(crate) fn encode_uint(&self, value: &ABIValue) -> Result<Vec<u8>, ABIError> {
        match self {
            ...
            _ => Err(ABIError::EncodingError { message: "ABI type mismatch, expected uint".to_string() }),
        }
    }
    ...
    pub(crate) fn decode_uint(&self, bytes: &[u8]) -> Result<ABIValue, ABIError> {
        match self {
            ...
            _ => Err(ABIError::DecodingError { message: "ABI type mismatch, expected uint".to_string() }),
        }
    }
}
