...
class PythonFreezeAdapter(AssetFreezeTestAdapter):
    ...
    def try_transfer_frozen(
        self, from_addr: str, to_addr: str, asset_id: int, amount: int
    ):
...
#         test_environment["algod_client"], test_environment["signer_getter"]
#     )
...
#             f"Individual test failed: {test_result.name} - {test_result.error}"
#         )
